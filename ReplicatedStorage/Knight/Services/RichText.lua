...
richText.ColorShortcuts.DarkGray = Color3.new(70/255, 70/255, 70/255)
richText.ColorShortcuts.Purple = Color3.new(170/255, 0, 170/255)
...
richText.ImageShortcuts.Glass = 1523548823
...
defaults.TextScale = 1/3 -- If you want the frame to have a nominal count of n lines of text, make this value 1 / n. For four lines, 1 / 4 = 0.25.
...
defaults.AnimateStyleTime = 2 -- How long it takes for an entrance style to fully execute
...
function richText:New(frame, text, startingProperties, allowOverflow, prevTextObject)
	...
	local function playSound(sound_id) -- added by Ponchokings
		local sound = Instance.new("Sound",game.ReplicatedStorage:WaitForChild("Sounds"))
			...
			for _, frame in pairs(list) do
				...
				elseif stepGrouping == "Letter" then
					if frame:IsA("TextLabel") --[[and not (frame.Text == " ") ]]then
						...
						local i = 1
						while true do
							playSound("rbxassetid://915576050") -- added by Ponchokings
